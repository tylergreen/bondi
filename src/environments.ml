...
module TyVarSet = Set.Make(OrderedTyVars);; 
...
type i_term =
  ...
  | Tlet of term_variable * i_term * i_term (* let-terms *) 
...
(*> CPC *)
  | Tname of P_data.name_form * term_variable * int    (* Names for CPC patterns: name form, variable, index *)
...
type type_data = 
  | Synonym of i_type  
  | Class of (tyVar * int) option * (p_term -> p_term) * (i_term -> i_term)   
...
type global_value_env = (int * (value * (let_status * i_type))) list TMap.t
...
let classTyStringLength = String.length classTyString 
let classConstructorString = "" (* "_constructor" *)
...
let rec type_of_class (str,n) zs rest = 
  ...
  try 
    ...
    | (_,Synonym ty) -> ty 
    | (n1,Class(Some (TyVar str0,n0),_,_)) -> 
	...
	begin
	  try 
	    match gTyEnvFind n tyv with 
	      _,Class(Some (tyv0,n0),_,_) -> pattern_of_class (tyv0,n0) local_pat
	    | _,Class(None,_,_) -> local_pat
...
;;
 ...
 let format_tyvarset tyvs =
 ps "{";
 TyVarSet.iter (fun x -> ps (string_of_tyvar x ^ ","))        tyvs ;
 ps "}";
 flush stdout
	...
	match split_apF [] ty with 
	  (TyC (TyVar str1,n1),zs1) -> 
	    if  true (* list_equal (zs,zs1) !!! *) &&
	      String.length str1 > classTyStringLength &&  
	      String.sub str1 (String.length str1 -classTyStringLength) classTyStringLength = 
	      classTyString
	    then 
	      let tyv1 =  TyVar (String.sub str1 0  (String.length str1 -classTyStringLength)) in 
	      try 
		match gTyEnvFind  n1 tyv1 with 
		  (_,Class(Some p,_,_)) when p = (tyv,n) -> class_path  (tyv1,n1) zs rest 
		| _ ->  (tyv,n,ty0)
	      with _ ->  (tyv,n,ty0)
	    else  (tyv,n,ty0)
	| _ -> (tyv,n,ty0)
...
let restore_class ty0 = 
  match ty0 with 
	...
	match split_apF [] main with 
	  (TyC(TyVar str,n),zs) -> 
	    if String.length str > classTyStringLength &&  
	      String.sub str (String.length str -classTyStringLength) classTyStringLength = 
	      classTyString
	    then 
	      let tyv = TyVar (String.sub str 0  (String.length str -classTyStringLength)) in 
	      begin 
		try 
		  match gTyEnvFind n tyv with 
		    n0,Class(None,_,_) -> 
