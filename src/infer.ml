...
let rec unify_p fixed precision sub ty0 ty1 = 
  ...
  *) 
  ...
  then sub 
   ...
   match (ty2,ty3) with
	...
	unify_p fixed precision sub1 ty f1 
   ...
   | (TyV (x,_),_)  -> addSubCheck sub x ty3
	...
	unify_p fixed precision 
	  (unify_p fixed Exact sub g0 g1) f0 f1
	...
	unify_p fixed Exact 
	  (unify_p fixed Exact sub g0 g1) f0 f1
    | SuperF(g0,f0), _ when precision = LessThan -> unify_p fixed LessThan sub f0 ty3
    | (Linty f1,Linty f2)
    | (Ref f1, Ref f2) 
    | (Array f1, Array f2) -> unify_p fixed Exact sub f1 f2 
    | (Quant(x1,f1), Quant(x2,f2)) ->
	let (sub1,x) = freshen_tyvar2 x1 x2 in 
	let sub2 = unify_p(x::fixed) precision sub (applySub sub1 f1) (applySub sub1 f2) in 
	if avoid x sub2 
	then sub2
...
let rec subunify fixed sub ty0 ty1 = unify_p fixed LessThan sub ty0 ty1 
...
let rec inf_linear p (sEnv: scheme_env) fixed tyEnv theta sub pty_opt status = 
  ...
  | Ptvar x -> 
	...
	let (n,(ty,status1)) = 
	  ...
	  with Not_found -> 
		...
		(n,(_,(status2,sch))) -> (n,(inst_tyscheme sch,status2))
  ...
  | Pwildcard str -> 
    let wild = Twildcard str in 
      begin
	match str with 
	  "" -> (sub,wild,delta0,tyEnv,pty)
	| "Int" 
	| "Float" 
	| "Char" 
	| "String"  -> (subunify [] sub (cvar str) pty,wild,delta0,tyEnv,pty)
	| "ref"  -> 
	    let x = TyV(nextTypeVar(),0) in 
	    (subunify [] sub (Ref x) pty,wild,delta0,tyEnv,pty)
	| "array"  -> 
	    let x = TyV(nextTypeVar(),0) in 
	    (subunify [] sub (Array x) pty,wild,delta0,tyEnv,pty)
 ...
 | Poper("where",[q;test]) -> 
  ...
  | Ptyped(p0,(Pnestedclass (str,_,_))) -> 
	...
	in inf_linear (Poper("as",[p0;the_pat])) sEnv fixed tyEnv theta sub (Some pty) status 
...
(*> CPC *)
  ...
  | Pname _ -> (fun _ _ _ _ -> typeError [] "Unexpected p_data name, aborting.")
(*< CPC *)
...
and infer_assign u v sEnv fixed sub0 expectedTy = 
  ...
  let (sub1,u1) = inf u sEnv fixed sub0 (Ref dty) (* not infer_field? *) 
  in   
  let (dty1,bound) = inst_tyscheme_with_bvars (clos_ty sub1 sEnv dty) in 
  let fixed2 = append bound fixed in 
  let (sub2,v2) = inf v sEnv fixed2 sub1 dty1 in 
  let sub3 = subunify fixed2 sub2 comm expectedTy
  in 
...
and infer_let = function 
  ...
  | Recursive -> infer_letrec 
  | Extensible -> infer_letext
  | Method -> infer_letmethod
  | Discontinuous -> infer_letdiscontinuous
...
and infer_letrec param u t sEnv fixed sub0 expectedTy = 
...
and infer_letdiscontinuous param u t sEnv fixed sub0 expectedTy = 
  ...
  if get_mode "types" = Show_off || 
     let _ = subunify fixed1 sub2 (clos_ty sub2 sEnv inst_ty) (clos_ty sub2 sEnv sch) in true 
  then (sub2,Tletdiscontinuous(Var x,u1,t2))
  else 
    typeError [applySub sub2 inst_ty] 
      "instantiates bound type variables in inference"
...
(* A simplistic approach! This function adds all the binding names
 * of a concurrent case to the substitution environment (while also
 * converting the pattern to an i_term) and then does inference for
 * the body. Note that this may all go horribly wrong, it is a dirty,
 * dirty hack! *)
and infer_cpc_case patt body sEnv fixed sub0 expectedTy =
  let getIndex x =
    try let _ = TMap.find x sEnv in 0
    with Not_found ->
          try match envFind 0 x globalVEnv with
              (n,(_,(_,_))) -> n
          with Not_found -> termError [Tvar (x,0)] "is not recognised"
  in
  let rec transform_cpc_pattern env = function
  | Pname(Binding,x) when TMap.mem (Var x) env -> termError [Tvar(Var x,0)] "is a duplicate binding symbol"
  | Pname(Binding,x) -> (TMap.add (Var x) (TyV(nextTypeVar(),0),Simple) env,Tname(Binding,Var x,0))
  | Pname(form,x) -> (env,Tname(form,Var x,getIndex (Var x)))
  | Papply(l,r) -> let (env1,l1) = transform_cpc_pattern env l in
                   let (env2,r2) = transform_cpc_pattern env1 r in
                   (env2,Apply(l1,r2))
  | _ -> typeError [TyV(nextTypeVar(),0)] "hit a malformed pattern for a process, aborting"
  in
  let (sEnv0,patt1) = transform_cpc_pattern TMap.empty patt in
  let sEnv1 = TMap.fold TMap.add sEnv0 sEnv in
  let (sub2,body2) = inf body sEnv1 fixed sub0 (TyV(nextTypeVar(),0)) in
  let sub3 = subunify fixed sub2 (cvar "Unit") expectedTy in
  (sub3,Pcase(patt1,body2))
(*< CPC *)
;;
