...
(*< CPC *)
  ...
  | Tletdiscontinuous(x,u,s) -> evalLetExt vEnv x u s
...
(*> CPC *)
(* Evaluate a thread seperately, store the thread ID in a queue to clean up later. *)
...
(* Do some optimisations and hacks to add replicating processes... *)
and eval_replication pEnv = function
  ...
  | Prll(p1,p2) -> let _ = (eval_replication pEnv p1,
                   ...
                   Vdatum Datum.Un
(*  | Prll(p1,p2) -> ignore (eval(pEnv,Repl(p1)));eval(pEnv,Repl(p2)) *)
  (* !(!P) is close enough to !P that we use the latter. *)
  | Repl(p) -> eval_replication pEnv p
(*  | Repl(_) as p -> eval(pEnv,p) *)
...
(* Determines if a pattern is communicable, i.e. can be
 * bound. Alternatively: has no protected or binding names. *)
let rec communicable = function
  | Tname(P_data.Variable,_,_) -> true
  | Tname(_,_,_) -> false
  | Apply(l,r) -> communicable l && communicable r
  | _ -> false
;;

(* We have bound a communicable pattern, strip out CPC details. *)
let rec unCPCPattern = function
  | Tname(_,v,n) -> Tvar(v,n)
  | Apply(l,r) -> Apply(unCPCPattern l,unCPCPattern r)
...
let rec unify1 (pEnv,qEnv) (subp,subq) = function
  ...
  | (Tname(P_data.Variable,pv,pn),Tname(P_data.Variable,qv,qn))
  | (Tname(P_data.Variable,pv,pn),Tname(P_data.Protected,qv,qn))
  | (Tname(P_data.Protected,pv,pn),Tname(P_data.Variable,qv,qn))
  | (Tname(P_data.Protected,pv,pn),Tname(P_data.Protected,qv,qn))
          -> if evalVar pEnv pv pn = evalVar qEnv qv qn then Some (subp,subq) else None
  (* When one pattern is binding, if the other is communicable then set up the substitution *)
  | (Tname(P_data.Binding,bp,_),qv) when communicable qv
          -> Some (TMap.add bp (eval (qEnv,(unCPCPattern qv))) subp,subq)
  | (pv,Tname(P_data.Binding,qb,_)) when communicable pv
          -> Some (subp,TMap.add qb (eval (pEnv,(unCPCPattern pv))) subq)
  (* When both sides are applications/compound patterns then check their
   * respective components. *)
  | (Apply(p1,p2),Apply(q1,q2)) ->
          begin
            match unify1 (pEnv,qEnv) (subp,subq) (p1,q1) with
            | Some (subp1,subq1) -> unify1 (pEnv,qEnv) (subp1,subq1) (p2,q2)
            | None -> None
          end
  (* When either one is an application and the other is a variable or protected
   * name. Find the value of the name (if it has one), and if that is an application
   * (i.e. a data structure) then create such information in the environment and
   * build a dummy compound pattern preserving the variable/protected status. *)
  | (Apply(p1,p2),Tname(qt,qv,qn)) when qt != P_data.Binding ->
          begin
            ...
            | Vapply(qVal1,qVal2) -> let qv1 = nextvar () in
                                     ...
                                     let q1 = Tname(qt,qv1,0) in
                                     let q2 = Tname(qt,qv2,0) in
                                     unify1 (pEnv,qEnv1) (subp,subq) (Apply(p1,p2),Apply(q1,q2))
            | _ -> None
          end
  | (Tname(pt,pv,pn),Apply(q1,q2)) when pt != P_data.Binding ->
          begin
            ...
            | Vapply(pVal1,pVal2) -> let pv1 = nextvar () in
                                     ...
                                     let p1 = Tname(pt,pv1,0) in
                                     let p2 = Tname(pt,pv2,0) in
                                     unify1 (pEnv1,qEnv) (subp,subq) (Apply(p1,p2),Apply(q1,q2))
            | _ -> None
          end
 ...
 * with it's ID (qid), environment (qE), processes (q) against
 * every other process (pE,p) in the environment. Result is
 * either false if no interaction and the input, or true if
 * there is a interaction with the list of processes IDs to
 * remove from the environment. *)
let foldinteract ppid (pl,pul,pE,p) (succ,qpid,qE,q,ks) =
  if succ || not (pl ())
  then (succ,qpid,qE,q,ks)
  else  match interactable pE qE p q with
        | Some cont -> (cont (); (true,qpid,qE,q,to_clean ppid ks p))
        | None -> (pul();(succ,qpid,qE,q,ks))
;;
