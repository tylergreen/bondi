...
module TyVarSet: Set.S with type elt = OrderedTyVars.t
module TyMap :
  sig
...
type i_term =
  ...
  | Tlet of term_variable * i_term * i_term 
...
(*> CPC *)
  | Tname of P_data.name_form * term_variable * int
...
type type_data = 
  | Synonym of i_type  
  | Class of (P_data.tyVar * int) option * (P_data.p_term -> P_data.p_term) * (i_term -> i_term)   
...
type global_value_env =
      (int * (value * (P_data.let_status * i_type))) list TMap.t
      (* dec time, location,   status,         type *) 
...
val classConstructorString : string 
