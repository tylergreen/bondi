...
%token STATIC SUPER THEN TO TRUE TYPE UN VIEW WHERE WHILE WITH
...
process:
  ...
  | REST identifierList IN process      { multirest $2 $4 }
  | CPC compoundPattern RARROW pTerm    { Ppcase ($2,$4) }

/* CPC name patterns are names that can be binding, protected or variable. */
namePattern:
...
/* Identifier list, used for restrictions. */
identifierList:
  | L_IDENT                   { [$1] }
  | L_IDENT identifierList    { $1::$2 }
