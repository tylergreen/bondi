...
let declare is_attribute identifier source =
  ...
  let (inferredTerm,inferredType) = 
    ...
    | Method -> 
	let (t1,ty1) = infer source expectedTy in 
	begin 
	  match ty1 with 
	    Funty(pty,sty) when is_simple_method_type [] pty sty -> (t1,funty pty sty) 
  ...
  then pf "no evaluation done" 
      ...
      let theScheme = clos_ty idSub TMap.empty inferredType in 
	...
	| _ -> theVal
      in 
      let closed = (theValue,(is_attribute,theScheme)) 
      in
...
let add_decl is_att thefun pattern pty_opt body = 
  try 
    let (_,(default,(att,sch))) =  envFind 0 (Var thefun) globalVEnv in 
    if is_att == att
    then 
      let sch2 = add_case_method thefun (None,pattern,pty_opt,body) sch in 
      match is_att with 
	Method -> envAdd (Var thefun) 0 (default,(Method,sch2)) globalVEnv  (*  !declaration_counter ?? *) 
      | _ ->  () 
...
let add_case_function constructors (x,(theta,p,ty_opt,s)) = 
  ...
  then 
    ...
    | (_,(_,(Discontinuous,_)))  -> add_outer_decl x p ty_opt s 
...
let declare_class cl_str zeds super_opt (fds,mds) add_cases = 
  ...
  in 
  let constructorString = cl_str ^ classConstructorString in 
  ...
  in 
  let (class_info,pattern) = 
    match super_opt with 
      None -> Class(None,local_pat,local_new),local_pat (Pwildcard "")
    | Some super_str -> 
	try 	
	  match gTyEnvFind ctr (TyVar super_str) with 
	  | (super_n,Class(_,whole_pattern,whole_new)) -> 
	      let pattern_maker x = whole_pattern (local_pat x) in 
	      let new_term x = whole_new (local_new x)
	      in  
	      Class(Some (TyVar super_str,super_n),pattern_maker,new_term),pattern_maker (Pwildcard "")
...
(* use convert_type to simplify above ? *) 
